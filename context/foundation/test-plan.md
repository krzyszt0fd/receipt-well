...
| #2 | Protected endpoints reject a missing/invalid token and an empty/missing `oid` with 401 — not 200, not 500 | "The gate is global so every endpoint is automatically covered" | Where `[Authorize]` / the auth middleware is applied and how a new endpoint opts in | Integration (request with no token / bad token / valid token) | Asserting only the happy authenticated case |
