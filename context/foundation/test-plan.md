...
| 8 | A user can delete another user's receipt (IDOR at delete), or a delete does not fully remove data from both stores, leaving an orphaned blob or a resurrected ghost document | High | Medium | this change (`receipt-delete`) + precedent from Risk #1 (IDOR) and Risk #7 (honest-5xx) |
...
| #8 | A cross-user delete is rejected before any side effect, a blob-delete failure after a successful Search-delete still surfaces as an honest 500, and a receipt deleted mid-extraction never gets a resurrected ghost document in the index | "Delete only needs to check the caller is authenticated"; "the Search delete succeeding is enough, the blob is a minor detail" | How delete's ownership check differs from confirm's prefix check and query's filter (fetch-then-compare, since the id-only route carries no user-prefixed path); the Search-delete-then-blob-delete ordering and what an in-flight extraction does after its receipt is deleted | Two-identity ownership integration test + honest-500 failure-shape test on blob-delete failure + unit test on the extraction-pipeline existence guard | Single-user fixture (hides the IDOR leak); asserting only the happy-path delete |
...
**Receipt delete (2026-07-05):**
- **A third ownership-check shape.** Confirm checks ownership via a blob-name prefix (`stagingBlobName.StartsWith($"{userId}/")`); query checks it via a Search filter (`UserId eq '{userId}'`); delete introduces a third pattern — fetch-then-compare — because the `DELETE /receipts/{id}` route carries only an id, no user-prefixed path. `ReceiptDeleteService` fetches the Search document first, checks its `UserId` against the caller, and only then deletes.
- **Search-then-blob delete ordering, honest-500 on the second step.** The Search document is deleted first (the side effect that "counts" for visibility); the blob delete follows and is idempotent (`DeleteIfExistsAsync`). A blob-delete failure after a successful Search-delete propagates as a real 500 (logged at Error, orphaned blob flagged for manual reconciliation) rather than being swallowed — same honest-5xx contract as the confirm flow's non-atomic writes.
- **Ghost-document guard closes a real but narrow race.** `ReceiptStore.SetReadyAsync`/`SetErrorAsync` now call `GetByIdAsync` before `MergeOrUploadDocumentsAsync`; if the receipt was deleted while extraction was in flight, the merge is skipped instead of falling back to Upload semantics and creating an orphaned, mostly-empty document invisible to every real user's query filter. The guard narrows the race window (a delete between the existence check and the merge is still possible) rather than eliminating it — accepted as a residual, rare risk.
